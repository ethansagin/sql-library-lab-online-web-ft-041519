...
def select_series_title_with_most_human_characters
  "SELECT Series.title FROM series JOIN Characters ON Series.id = Characters.series_id WHERE Characters.species = 'human' GROUP BY Series.title, Characters.species HAVING COUNT(Characters.species) ORDER BY Characters.species DESC LIMIT 1;"
end
